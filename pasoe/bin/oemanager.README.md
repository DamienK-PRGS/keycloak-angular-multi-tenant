...
The following represents the default usage for all tasks which can be executed using the CLI tools and is the standard output when the `oemanager.[bat|sh]` command is run without a task name.
     ...
     oemanager startup  - Use TCMAN to start the PAS instance
                          [OPTIONAL] timeout=300 - Time (seconds) to wait for a proper startup

     oemanager shutdown - Use TCMAN to stop the PAS instance
                          [OPTIONAL] timeout=300 - Time (seconds) to wait for a proper shutdown
     ...
     oemanager status - [RO] Obtain MSAgent/connection status information for an ABL App
                        [REQUIRED]  ablapp=oepas1 - ABL Application name, defaulting to the instance name if not changed
                        [OPTIONAL] basemem=819200 - Minimum memory threshold (bytes) to consider as 'unused' agent sessions

     oemanager stacks - [RO] Obtain stack information for all MSAgents for an ABL App
                        [REQUIRED] ablapp=oepas1       - ABL Application name, defaulting to the instance name if not changed
                        [OPTIONAL]    pid=[AGENT_PID]  - Limit stack information to a specific MSAgent process ID
                        [OPTIONAL] sessid=[SESSION_ID] - Limit stack information to a specific ABL Session ID
                                                           Requires specifying an MSAgent via pid=[AGENT_PID]
     ...
     oemanager locks  - [RO] Display database users and their table locks related to an MSAgent-Session
                        ...
                        [REQUIRED] ablapp=oepas1    - ABL Application name, defaulting to the instance name if not changed
                        [REQUIRED]     pf=[PF_NAME] - PF file to use for database connection(s)
     ...
     oemanager close   - Perform a 'soft restart' of an ABL App (runs: status, flush + trimhttp + stop, status)
                         ...
                         [REQUIRED] ablapp=oepas1        - ABL Application name, defaulting to the instance name if not changed
                         [REQUIRED] webapp=[WEBAPP_NAME] - WebApp for Tomcat Manager to terminate active client sessions
                                     The given WebApp is expected to be associated with the provided ablapp name
                         [OPTIONAL]  sleep=1             - Sleep time in minutes after stop, prior to final 'status' output

     oemanager reset   - Reset an aspect of each MSAgent for an ABL App
                         [REQUIRED]    ablapp=oepas1 - ABL Application name, defaulting to the instance name if not changed
                         [REQUIRED] resettype=stats  - Type may be one of the following [stats|logs]

     oemanager stop    - Gracefully stop one or all MSAgents (+stacks output) for an ABL App
                         [REQUIRED]     ablapp=oepas1 - ABL Application name, defaulting to the instance name if not changed
                         [OPTIONAL] waitfinish=120000 - How long to wait (milliseconds) if the MSAgent is busy serving a request
                         [OPTIONAL]  waitafter=60000  - Additional time to wait (milliseconds) before killing [hard stop] the MSAgent
                         [OPTIONAL]        pid=[AGENT_PID] - Numeric process ID for a specific MSAgent to be stopped
     ...
     oemanager trimsingle - Trim a single ABL Session (via the Agent Manager) for a specific MSAgent
                            [REQUIRED]       ablapp=oepas1       - ABL Application name, defaulting to the instance name if not changed
                            [REQUIRED]          pid=[AGENT_PID]  - Numeric process ID of the MSAgent for context
                            [REQUIRED]       sessid=[SESSION_ID] - Numeric ID for the ABL Session to be stopped
                            [OPTIONAL] terminateopt=0            - Termination Option: 0=graceful, 1=forced, 2=finish+stop
     ...
     oemanager trimhttp   - Trim one or all Client HTTP Sessions (via the Session Manager) for an ABLApp + WebApp
                            ...
                            [REQUIRED]       ablapp=oepas1        - ABL Application name, defaulting to the instance name if not changed
                            [REQUIRED]       webapp=[WEBAPP_NAME] - WebApp for Tomcat Manager to terminate active sessions
                                              The given WebApp is expected to be associated with the provided ablapp name
                            [OPTIONAL]       sessid=[SESSION_ID]  - Alphanumeric Client Session ID to be stopped
...
The output for any generated files from available tasks will be placed in the location referred to by the `outdir` value. This may be one of 2 built-in options, `BIN` or `TMP` for the bin directory (`CATALINA_BASE/bin`) or temp folder (`CATALINA_BASE/temp`), respectively. Alternately, you may specify a full path to use for all output.
