...
findJar() {
    jarDir=$1
    jarName=$2
    jars=`ls $jarDir/$jarName-[0-9\.]*\.jar 2>/dev/null`
    jars=`echo "$jars" |  tr "\n" " "`
for ff in  $jars; do
  jar=$ff 
done
...
findJar $binDir "deployablsvc"
deployablsvc=$jar
findJar $commonLib "logback-classic"
logbackClassicJar=$jar
findJar $commonLib "oelogging"
oeloggingJar=$jar
...
command="$pasoeJmxCall $_jvmArgs -DCATALINA_BASE=$CATALINA_BASE -DCATALINA_HOME=$CATALINA_HOME -Dorg.apache.cxf.Logger=$logger -Dlogback.configurationFile=$logbackFile -cp ./:$deployablsvc:$logbackClassicJar:$oeloggingJar com.progress.appserv.util.deployablsvc.MergeCsv $args" 
if [ ! "$dbg" = "" ]; then
